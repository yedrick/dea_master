...
class Func {
    ...
    public static function upload($file,$folder,$code=null,$options=[]) {
        $filename = str_replace('-', '', Str::uuid()->toString()).($options['extension'] ?? 'jpg');
        $configs=[
            [
                'code' => 'original',
                'type' => 'original',
    ...
    public static function processAndStore($file,$folder,$size,$filename,$code=null,$options) {
        ...
        // }
        $newFilename = public_path($folder . '/' . $size['code'] . '/' . $filename . '.' . $extension); // Guardar directamente en public
        $path = public_path($folder . '/' . $size['code']); // Crear la ruta de la carpeta dentro de public
        if (!File::exists($path)) {
